...
class IncidentTests(BaseTest):
    ...
    def test_get_incident_byId(self):
        ...
        response = self.app.get('/api/v2/incidents/1', headers = {'Authorization': 'Bearer {}'.format(self.user1_token()),
    ...
    def test_update_incident_description(self):
        ...
        result = json.loads(response.data)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(result[0]['Message'], 'Updated incident comment successfully!')
    ...
    def test_fail_delete_incident(self):
        self.create_incident()
        response=self.app.delete('/api/v2/incidents/200',  headers={'Authorization': 'Bearer {}'.format(self.user1_token()),
                               'content-type': 'application/json'})

...
document.getElementById('userIncident').addEventListener('click', getUserIncidents);
...
function getUserIncidents(e){
  e.preventDefault();
  let token = sessionStorage.getItem('token');
  fetch('https://issaireporterv2.herokuapp.com/api/v2/userincidents', {
    method: 'GET',
    headers: {
      'Authorization': 'Bearer ' + token
      }
      ...
      .then((data) => {
          if (data.Error) {
              ...
              let output = `<h2>These are all your incidents:</h2>`;
              data[0].Data.forEach(function (incident) {
                  output += `
            <ul class="list-group mb-3">
                <li class="list-group-item"><strong>INCIDENT ID: </strong>${incident.incident_id}</li>
            ...
            `;
              ...
              document.getElementById('output0').innerHTML = output;
              alert(data[0].Message)
          }
      })
}